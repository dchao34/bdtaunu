...
double physics_sensitivity(const dvec_t& dec_values, const ivec_t& ty);

// evaluation function pointer
// You can assign this pointer to any above prototype
double (*validation_function)(const dvec_t&, const ivec_t&) = gmean;
...
double physics_sensitivity(const dvec_t& dec_values, const ivec_t& ty) {
	...
	if(tp + fp == 0){
		...
		phys_sense = tp / sqrt((double) (tp + fp));
	printf("Physics Sensitivity (S/sqrt(S+B))= %g%% (%d/%d)\n", phys_sense, tp, tp + fp);
