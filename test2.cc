...
#include "candidate_selection/YCandSvmAdapter.h"
...
void configure_sql_qeury(
  ...
  sql_query = "SELECT "
              ...
              "WHERE ml_sample='explore' AND division=1 AND cand_type=4;";
...
int main(int argc, char **argv) {
  ...
  YCandFeatureExtractor *extractor = 
    YCandFeatureExtractorFactory::CreateExtractor(kDDrho);

  DDrhoSvmAdapter svm_scorer;
  ...
  ofstream libsvm_file("ddrho_explore.dat", ofstream::out);
  ofstream predict_output("ddrho_explore.predicted", ofstream::out);
